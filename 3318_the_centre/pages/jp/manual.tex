...
\newfontfamily\japfont[
  Path = fonts/,
  Scale=1.0
] {Eras-Bold-ITC.ttf}

\setCJKmainfont[ Path = fonts/, BoldFont = SourceHanSans-Bold.otf, ItalicFont = SourceHanSerif-Light.otf ]{SourceHanSans-Normal.otf}
...
\titlecontents{subsubsection}[2.8pc]
{\footnotesize}
{\begin{tikzpicture}[baseline={([yshift=-.8ex]current bounding box.center)},]
\node [
  thick, draw=none, fill=none, rectangle, rounded corners
...
\begin{titlepage}
  \begin{tikzpicture}[overlay,remember picture,line width=5pt]
      ...
      \node (title_img) at ($(current page.south)!0.33!(current page.north)$) {\includegraphics[width=19cm]{the-centre-monotone.png}};
  ...
  %     % red box at 0.75\paperheight
      ...
      %      {\amatica \today};
          ...
          \node [
            ...
            anchor = south east,
            align = left,
          ] at (current page.south east) {\copyright 2022 1V/Oct. All rights reserved.
          };
...
\ifdefined\USERMANUAL%
  ...
  \newpage\input{pages/software_update_JP.tex}
  \newpage\input{pages/calibrate_JP.tex}
  \newpage\input{pages/whats_in_the_box_JP.tex}
  \newpage\input{pages/getting_started.tex}  
  \newpage\input{pages/clocks_JP.tex}
  \newpage
  \part{システム概要}
  \newpage\input{pages/overview_JP.tex}
  \newpage
  \part{操作}
  \newpage\input{pages/multi_patch_JP.tex}
  \clearpage
  \newpage
  \part{モジュール}
\else
...
\ifdefined\USERMANUAL%
  ...
  \part{補足}
  \newpage
  \section{ファイル形式}\label{section:fileformats}
  \subsection{VXP - Patch File Format}
  \paragraph*{}%
  \textbf{VXP} ファイル形式には、パッチ、接続されたモジュール、入力、出力、および設定のすべての定義が含まれています。これは The Center のデータの基本的なバイナリ形式であり、モジュールの定義と完全に結びついています。 参照: \underline{\nameref{section:oppatch}}
  \subsection{VXS - Multi-Patch format}
  \paragraph*{}%
  \textbf{VXS} 形式には、複数のパッチ (4) とエフェクト パッチが含まれています。 VXS フォーマットは、Multi-Patch (Set) モードからのみ開いて保存できます。 参照: \underline{\nameref{section:multipatch}}
