...
\def\vcoset{
  {{Audio Output}/{Audio Output of VCO}/{Selects audio output destination for VCO.
...
\def\vcoinp{
  ...
  \newline$\blacksquare$  Adding a modulator to CV input of FM and having it running at certain ratio of frequency to the actual VCO will create interesting sound texture}},
  ...
  \newline$\bigstar$ By connecting two VCOs detuned slightly through VCO1 Ooutput: Reset to VCO2 Input: Hard Sync we can create ver rich texture of simple oscillation}},
...
\def\vcoout{
    {{OSC}/{Output of VCO}/{Output of oscillator signal}},
...
\newcommand{\modvco}{
  ...
  \paragraph*{}
Voltage Controlled Oscillator is a sound source generating oscillations of waveforms.
  
  \showrefcardsection{vco}

  \subsection{Waveforms}
  \paragraph*{}VCO takes basic shapes: Sine, Triangle and Square and oscillates the single phase of those waveforms at given frequency. Each \textbf{Waveform} can be modified by bending it's pivot params with the Input:Skew CV input. Applying Skew parameter to triangle turns it into either \textit{Ramp} or \textit{Saw}. For Square, Skew parameter adjusts PWM (Pulse Width Modulation) which modifies length of square pulse. For Sine, Skew parameter changes the ratio of phase for positive and negative parts of Sine wave.


  \newpage
  \section*{\modname}  
  \tableofoptionsnew{\vcoset}{{Settings}}%
  \tableofoptionsnew{\vcoinp}{{Inputs}}%
  \tableofoptionsnew{\vcoout}{{Outputs}}%
}
