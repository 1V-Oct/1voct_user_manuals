...
% Use \providecommand to define the family command. This is more robust for multi-pass compilations.
  \IfStrEqCase{\LANG}{
    {zh-hans}{%
        \ifcsdef{titlefontcjk}{}{\newfontfamily\titlefontcjk{RacingSansOne-Regular.ttf}[Path=../fonts/]}
...
% \newcommand\tocboxedpart{{%
%   \begin{tikzpicture}[remember picture, overlay]
%     \fill[black]
%       (current page.west |- 0,0.8em)
%       rectangle
%       (current page.east |- 0,-0.5em);
%   \end{tikzpicture}%
% }}
...
% \newcommand\tocboxedpart[1]{%
%   \begin{tikzpicture}[inner sep=0pt, inner ysep=0.4ex, baseline]
%     % The text node first (this determines the bounding box height)
%     \node[
%       inner xsep=3mm,     % left padding
%       inner ysep=0.6ex,   % vertical padding inside the bar
%       text=white,
%       anchor=base west,
%       font=\titlefontfamily\fontsize{14}{14}\selectfont
%     ] (tocnode) at (0,0) {\thecontentslabel \hskip 0.75ex #1};

%     % Full-width black background using the node's bounding box extended to page edges
%     \begin{pgfonlayer}{background}
%       \fill[black] 
%         (tocnode.north west -| current page.west) 
%         rectangle 
%         (tocnode.south east -| current page.east);
%     \end{pgfonlayer}
%   \end{tikzpicture}%
% }

% \newcommand\tocboxedpart[1]{%
%   \begin{tikzpicture}[inner sep=0pt, baseline=0pt]
%     % Text node - this determines the natural line height
%     \node[
%       inner xsep=3mm,      % left padding
%       inner ysep=1ex,    % controls bar thickness - adjust if needed
%       text=white,
%       anchor=base west,
%       font=\titlefontfamily\fontsize{14}{14}\selectfont
%     ] (tocnode) at (0,0) {\thecontentslabel \hskip0.75ex #1};

%     % Full-width black bar using the text node's height
%     \begin{pgfonlayer}{background}
%       \fill[black] 
%         (tocnode.north west -| current page.west) 
%         rectangle 
...
%     % The node contains ONLY the title text (white on black)
%     \node[
%       inner xsep=3mm,      % left padding (reduce to 2mm if you want more space)
%       inner ysep=0.8ex,    % vertical thickness of the black bar
%       text=white,
%       anchor=base west,
%       font=\titlefontfamily\fontsize{14}{14}\selectfont
%     ] (tocnode) at (0,0) {\thecontentslabel \hskip0.75ex #1};
...
\newcommand\tocboxedpart[1]{%
  \leavevmode
  \begin{tikzpicture}[baseline=(tocnode.base), inner sep=0pt]
    % Text node with title only
    \node[
      inner xsep=3mm,      % left padding
      inner ysep=0.85ex,   % bar thickness (adjust 0.6ex–1.2ex)
      text=white,
      anchor=base west,
      font=\titlefontfamily\fontsize{14}{14}\selectfont,
      inner ysep=0.85ex
...
%     % Full-width black background - exactly matching the "Contents" header style
%     \fill[black] 
%       ([yshift=1.2ex]current page.west) rectangle 
%       ([yshift=-1.0ex]current page.east);

%     % White text with proper vertical centering and left padding
%     \node[
%       text=white,
%       anchor=base west,
%       font=\titlefontfamily\fontsize{14}{14}\selectfont,
%       inner xsep=3mm,   % left padding - adjust if needed
%       inner ysep=0pt,
%       outer sep=0pt
%     ] at ([xshift=0pt, yshift=0.1ex]current page.west) 
%       {\thecontentslabel \hskip0.75ex #1};
%   \end{tikzpicture}%
% }
...
\contentsmargin{2.5em}   % Try 3.8em to 4.5em — larger value = more space for page number
...
%   {\tocboxedpart}% 
%   {}
%   {\titlerule*[0.5pc]{.}\textcolor{white}{\large\thecontentspage}}   % ← important change

\titlecontents{part}[3.5pc]
  {\addvspace{10pt}\titlefontfamily\fontsize{14}{14}\selectfont}
  {\contentslabel{2.3em}}
  {\hspace*{-2.3em}}
  {\titlerule*[0.5pc]{.}\thecontentspage}

\titlecontents{section}[3.0pc]
{}%
...
\titlecontents{subsubsection}[6.8pc]
{}%
{\begin{tikzpicture}[baseline={([yshift=-.8ex]current bounding box.center)},]
\node [
  thick, draw=none, fill=none, rectangle, rounded corners
...
\input{../3318_the_centre/shared/modules.tex}
\input{../shared/commands.tex} % This should be loaded once, here.
